...
* `validateBuildProperties` - validates all properties required for specific platform. This includes check if current application identifier matches the CodeSigning identity for iOS operations, check if the specified device identifier (in case there's such) is included in the mobile provision, validation of the password, etc.
</br>
Definition:
